...
async function getBootstrapData(user) {
  ...
  const devices = (deviceRes.data || []).map(device => {
    ...
    return {
      ...
      streamUrl: reported?.telemetry?.cameraStreamUrl || reported?.telemetry?.streamUrl || device.stream_url,
      screenUrl: reported?.telemetry?.holoboxScreenUrl || device.stream_url,
      defaultLanguage: device.default_language === 'en' ? 'en' : 'vi',
